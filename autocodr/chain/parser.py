...
class CodeBlockOutputParser(BaseOutputParser[list[str]]):
    def parse(self, text: str) -> list[str]:
        return (
            ...
            if (
                codeblocks := [
                    codeblock.group(2).strip() for codeblock in re.finditer(r"```(\w+)(.*?)```", text, re.DOTALL)
                ]
            )
            else raiser(OutputParserException("No code blocks found in response"))
        )
...
class FilePathsOutputParser(BaseOutputParser[dict[str, list[str]]]):
    def parse(self, text: str) -> dict[str, list[str]]:
        return (
            ...
            if (json_text := re.search(r"```json(.*?)```", text, re.DOTALL))
            else raiser(OutputParserException("No JSON found in response"))
        )
