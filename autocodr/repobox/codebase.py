...
class CodeBase(CodeBox):
    ...
    async def astart(self, repository: Repository, access_token=None):
        await super().astart()
        await self.arun("import os; os.mkdir('.codebase'); os.chdir('.codebase');")
        # setup git credentials for pushing and forking
        await self.bash(
            f"git clone {repository.clone_url} .",
            "git config --global user.email 'github@autocodr.com'",
            "git config --global user.name 'AutoCodr'",
            "git config --global credential.helper store",
        )
        if access_token:
            await self.bash(
                "git config --global credential.helper "
                f"'!f() {{ echo username=git; echo password={access_token}; }}; f'"
            )
        await self.bash("git config --global push.default simple")

    async def bash(self, *commands: str) -> str:
        return (await self.arun("os.system('" + "; ".join(commands) + "')")).content

    async def read_file(self, relative_path: str) -> bytes | None:
        return (await self.adownload(relative_path)).content

    async def show_tree(self) -> str | None:
        return (await self.arun("os.walk('.')")).content

    async def show_file(self, relative_path: str) -> str | None:
        # TODO: check max string length
        return (await self.arun(f"open('{relative_path}').read()")).content

    async def file_exists(self, relative_path) -> bool:
        # TODO: test this
        return (await self.arun(f"os.path.exists('{relative_path}')")).content == "True"
    ...
    async def validate_file_paths(self, file_paths):
        ...
        return bool(
            (
                await self.arun(
                    "import re; all([re.search(r'.*\\..*', path) " f"is not None for path in {file_paths}])"
                )
            ).content
        )

    async def create_file(self, relative_path, content) -> bool:
        return relative_path in (await self.aupload(relative_path, content)).status
